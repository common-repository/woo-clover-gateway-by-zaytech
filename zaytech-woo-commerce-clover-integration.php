...
function zaytech_woocci_init() {
 ...
 : We require WooCommerce to be installed and active. You can download %s here.', 'zaytech_woocci' ), '<a href="https://woocommerce.com/" target="_blank">WooCommerce</a>' ) . '</strong></p></div>';
    ...
    if ( version_compare( WC_VERSION, WOOCCI_MIN_WC_VER, '<' ) ) {
        add_action( 'admin_notices', function () {
            echo '<div class="error"><p><strong>' . sprintf( esc_html__( 'WooCommerce Clover Payment Gateway by Zaytech requires WooCommerce %1$s or greater to be installed and active. WooCommerce %2$s is no longer supported.', 'zaytech_woocci' ), esc_html( WOOCCI_MIN_WC_VER ), esc_html( WC_VERSION ) ) . '</strong></p></div>';
        });
        return;
    }
